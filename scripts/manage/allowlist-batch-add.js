...
async function main() {
  ...
  console.log("Reading allow list status for addresses before adding")
  ...
  console.log("add addresses to whitelist: ", ADDRESSES.length)
  const tx = await vartContract.batchAddMembersToWhitelist(ADDRESSES);
  await tx.wait();
